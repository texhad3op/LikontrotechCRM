...
@Stateless(name = "GenerateInvoiceEJB")
public class GenerateInvoiceBean implements GenerateInvoiceLocal {
	...
	String lastName;
	Timestamp payBefore;
	boolean addDeliveringPrice;
	BigDecimal deliveringPrice;
	...
	private void addTitlePage(Document document) throws DocumentException,
		...
		preface.add(new Paragraph("PVM SĄSKAITA FAKTŪRA", catFont));

		addEmptyLine(preface, 1);
		preface.add(new Paragraph("Serija LKT Nr. "
				+ (real ? serialCodeLocal.getSerialCodeAndIncrement()
						: serialCodeLocal.getSerialCodeString()), simpleFont));
		...
		cell = new PdfPCell(new Paragraph("Prekių (paslaugų) tiekėjas:",
				smallFont));
		cell.setBorder(0);
		cell.setHorizontalAlignment(Element.ALIGN_LEFT);
		table.addCell(cell);
		cell = new PdfPCell(new Paragraph("Prekių (paslaugų) pirkėjas:",
				smallFont));
		cell.setBorder(0);
		...
		cell.setHorizontalAlignment(Element.ALIGN_LEFT);
		table.addCell(cell);
		cell = new PdfPCell(new Paragraph("Įmonės kodas: "
				+ company.getCompanyCode(), smallFont));
		cell.setBorder(0);
		cell.setHorizontalAlignment(Element.ALIGN_RIGHT);
		table.addCell(cell);

		cell = new PdfPCell(new Paragraph("PVM mokėtojo kodas: LT213455417",
				smallFont));
		cell.setBorder(0);
		cell.setHorizontalAlignment(Element.ALIGN_LEFT);
		table.addCell(cell);
		cell = new PdfPCell(new Paragraph("PVM mokėtojo kodas: "
				+ company.getVatCode(), smallFont));
		cell.setBorder(0);
		cell.setHorizontalAlignment(Element.ALIGN_RIGHT);
		table.addCell(cell);

		cell = new PdfPCell(new Paragraph(
				"Adresas: Minties g 48-88, LT-09220, Vilnius 51", smallFont));
		cell.setBorder(0);
		cell.setHorizontalAlignment(Element.ALIGN_LEFT);
		table.addCell(cell);
		cell = new PdfPCell(new Paragraph("Adresas: " + company.getAddress(),
				smallFont));
		cell.setBorder(0);
		cell.setHorizontalAlignment(Element.ALIGN_RIGHT);
		table.addCell(cell);
		...
		cell = new PdfPCell(new Paragraph("a/s: LT644010042400021439",
				smallFont));
		cell.setBorder(0);
		cell.setHorizontalAlignment(Element.ALIGN_LEFT);
		table.addCell(cell);
		...
		cell.setHorizontalAlignment(Element.ALIGN_LEFT);
		table.addCell(cell);
		cell = new PdfPCell(new Paragraph("Apmokėti iki: "
				+ Utils.getDateForDocument(payBefore), smallFont));
		cell.setBorder(0);
		cell.setHorizontalAlignment(Element.ALIGN_RIGHT);
		table.addCell(cell);
	...
	private void addFooter(Document document) throws DocumentException,
		...
		if (!new Integer(0).equals(discount)) {
			...
			cell.setHorizontalAlignment(Element.ALIGN_RIGHT);
			table.addCell(cell);
			cell = new PdfPCell(new Paragraph(discount.toString() + " %",
					smallFont));
			cell.setBorder(0);
			cell.setHorizontalAlignment(Element.ALIGN_RIGHT);
			table.addCell(cell);
		...
		parag.add(new Paragraph("Suma žodžiais: "
				+ SumAsWords.getSumAsWors(Double.valueOf(sumForGeneration)
						.floatValue()), smallFont));
		...
		table.setWidthPercentage(100);
		cell = new PdfPCell(new Paragraph("Sąskaitą išrašė:", smallFont));
		cell.setBorder(0);
		cell.setPaddingLeft(100);
		cell.setHorizontalAlignment(Element.ALIGN_LEFT);
		table.addCell(cell);

		cell = new PdfPCell(new Paragraph(position + " " + firstName + " "
				+ lastName, smallFont));
		cell.setBorder(0);
		cell.setHorizontalAlignment(Element.ALIGN_LEFT);
		table.addCell(cell);
		...
		table = new PdfPTable(2);
		table.setWidthPercentage(100);
		cell = new PdfPCell(new Paragraph("Sąskaitą gavo:", smallFont));
		cell.setBorder(0);
		cell.setPaddingLeft(100);
		cell.setHorizontalAlignment(Element.ALIGN_LEFT);
		table.addCell(cell);
	...
	private void addContent(Document document,
			List<CommercialDocumentLine> elements) throws DocumentException,
	...
	private void createTable(Paragraph parag,
			List<CommercialDocumentLine> elements) throws BadElementException,
			MalformedURLException, IOException {
		...
		for (CommercialDocumentLine commercialDocumentLine : elements) {
			...
			CatalogElement catalogElement = catalogLocal
					.find(commercialDocumentLine.getCatalogElementId());

			List<Object[]> objects = attributeValueLocal
					.getAttributValuesForInvoice(catalogElement);
			addProductInfo(i++, commercialDocumentLine, catalogElement,
					null == commercialDocumentLine.getRealPrice() ? 0
							: commercialDocumentLine.getRealPrice()
									.doubleValue(), objects,
					null == commercialDocumentLine.getQuantity() ? 0
							: commercialDocumentLine.getQuantity(), table);
		}
		if (addDeliveringPrice) {
			PdfPCell c2 = new PdfPCell(new Phrase(String.valueOf(i)));
			c2.setHorizontalAlignment(Element.ALIGN_CENTER);
			c2.setVerticalAlignment(Element.ALIGN_MIDDLE);
			table.addCell(c2);

			c2 = new PdfPCell(new Phrase("Transportavimo išlaidos, Lt be PVM", simpleFont));
			c2.setHorizontalAlignment(Element.ALIGN_LEFT);
			c2.setVerticalAlignment(Element.ALIGN_MIDDLE);
			c2.setColspan(3);
			table.addCell(c2);
			
			c2 = new PdfPCell(new Paragraph(String.format(Locale.getDefault(),
					"%.2f", deliveringPrice), simpleFont));
			c2.setHorizontalAlignment(Element.ALIGN_CENTER);
			c2.setVerticalAlignment(Element.ALIGN_MIDDLE);
			table.addCell(c2);
		}
	...
	private void addProductInfo(Integer Nr,
			CommercialDocumentLine commercialDocumentLine,
			CatalogElement catalogElement, Double Kaina,
			List<Object[]> objects, Integer Kiekis, PdfPTable table)
	...
	public void generatePdf(boolean real,
			List<CommercialDocumentLine> elements, Integer days,
